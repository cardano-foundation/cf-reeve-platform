...
public class AccountingCorePresentationViewService {
    ...
    private final ValidateIngestionResponseWaiter validateIngestionResponseWaiter;
        ...
        private static final Map<String, String> RV_FIELD_MAP = Map.of(
                        "id", "transactionId",
        ...
        public Pageable expandSorts(Pageable pageable, boolean mapRv) {
                ...
                pageable.getSort().forEach(order -> {
                        ...
                        if (RV_FIELD_MAP.containsKey(field) && mapRv) {
                                ...
                                newOrders.add(new Sort.Order(order.getDirection(), "rv." + rvField));
                        }
                });
        ...
        public ReconciliationResponseView allReconciliationTransaction(ReconciliationFilterRequest body,
                ...
                if (body.getFilter().equals(ReconciliationFilterStatusRequest.UNRECONCILED)) {
                        ...
                        Page<TransactionEntity> pagedTransactions = reconcilationRepository
                                        .findAllReconcilation(body.getFilter().name(), body.getSource(), pageable);
        ...
        public Either<Problem, Optional<BatchView>> batchDetail(String batchId,
                        List<TransactionProcessingStatus> txStatus,
                        Pageable page, BatchFilterRequest batchFilterRequest) {
                ...
                return Either.right(transactionBatchRepositoryGateway.findById(batchId).map(transactionBatchEntity -> {
                        Page<TransactionEntity> transactions = this.getTransaction(transactionBatchEntity, txStatus,
                                        finalPage,
                                        batchFilterRequest);

                        BatchStatisticsView statistic = BatchStatisticsView.from(batchId,
                                        transactionBatchEntity.getBatchStatistics().orElse(new BatchStatistics()));
                        FilteringParametersView filteringParameters = this
                                        .getFilteringParameters(transactionBatchEntity.getFilteringParameters());
        ...
        public Either<Problem, BatchsDetailView> listAllBatch(BatchSearchRequest body, Sort sort) {
                ...
                Either<Problem, List<TransactionBatchEntity>> transactionBatchEntitiesE = transactionBatchRepositoryGateway
                                .findByFilter(body, sort);
        ...
        public Either<Problem, Void> extractionTrigger(ExtractionRequest body) {
                ...
                return accountingCoreService.scheduleIngestion(fp, body.getExtractorType(),
                                Optional.ofNullable(body.getFile()),
                                body.getParameters());
        }
        ...
        public Either<List<Problem>, Void> extractionValidation(ExtractionRequest body) {
                ...
                return accountingCoreService.validateIngestion(userExtractionParameters, body.getExtractorType(),
        ...
        public List<TransactionProcessView> approveTransactions(TransactionsRequest transactionsRequest) {
                return transactionRepositoryGateway.approveTransactions(transactionsRequest)
                                ...
                                .map(txEntityE -> txEntityE.fold(
                                                txProblem -> TransactionProcessView.createFail(txProblem.getId(),
        ...
        public TransactionItemsProcessRejectView rejectTransactionItems(
                ...
                if (txM.isEmpty()) {
                        Either<IdentifiableProblem, TransactionEntity> errorE = transactionNotFoundResponse(
                ...
                Set<TransactionItemsProcessView> items = transactionRepositoryGateway
                                .rejectTransactionItems(tx,
                                                transactionItemsRejectionRequest.getTransactionItemsRejections())
                                .stream()
                                .map(txItemEntityE -> txItemEntityE.fold(
                                                txProblem -> TransactionItemsProcessView.createFail(txProblem.getId(),
                                                                txProblem.getProblem()),
        ...
        private Page<TransactionEntity> getTransaction(TransactionBatchEntity transactionBatchEntity,
                ...
                return accountingCoreTransactionRepository.findAllByBatchId(transactionBatchEntity.getId(), status,
                                ...
                                Optional.ofNullable(batchFilterRequest.getDateFrom()).orElse(LocalDate.of(1970, 1, 1))
                                                .atStartOfDay(),
                                Optional.ofNullable(batchFilterRequest.getDateTo()).orElse(LocalDate.now())
                                                .atStartOfDay(),
        ...
        private TransactionReconciliationTransactionsView getTransactionReconciliationView(
                ...
                try {
                        ...
                        log.warn("Invalid extractorType '{}' for transaction {}", transactionEntity.getExtractorType(),
                ...
                return new TransactionReconciliationTransactionsView(
                                ...
                                false, // TODO Hard coded value?
                                transactionEntity.getReconcilation()
                                                .flatMap(reconcilation -> reconcilation.getSource()
                                                                .map(TransactionReconciliationTransactionsView.ReconciliationCodeView::of))
                                                .orElse(TransactionReconciliationTransactionsView.ReconciliationCodeView.NEVER),
                                transactionEntity.getReconcilation()
                                                .flatMap(reconcilation -> reconcilation.getSink()
                                                                .map(TransactionReconciliationTransactionsView.ReconciliationCodeView::of))
                                                .orElse(TransactionReconciliationTransactionsView.ReconciliationCodeView.NEVER),
                                transactionEntity.getReconcilation()
                                                .flatMap(reconcilation -> reconcilation.getFinalStatus()
                                                                .map(TransactionReconciliationTransactionsView.ReconciliationCodeView::of))
                                                .orElse(TransactionReconciliationTransactionsView.ReconciliationCodeView.NEVER),

                                transactionEntity
                                                .getLastReconcilation()
                                                .map(reconcilationEntity -> reconcilationEntity.getViolations().stream()
                                                                .filter(reconcilationViolation -> reconcilationViolation
                                                                                .getTransactionId()
                                                                                .equals(transactionEntity.getId()))
                                                                .map(reconcilationViolation -> ReconciliationRejectionCodeRequest
                                                                                .of(
                                                                                                reconcilationViolation
                                                                                                                .getRejectionCode(),
                                                                                                transactionEntity
        ...
        private TransactionView getTransactionView(TransactionEntity transactionEntity) {
                ...
                return new TransactionView(
                                ...
                                transactionEntity.getReconcilation().flatMap(
                                                reconcilation -> reconcilation.getSource()
                                                                .map(TransactionView.ReconciliationCodeView::of))
                                                .orElse(TransactionView.ReconciliationCodeView.NEVER),
                                transactionEntity.getReconcilation().flatMap(
                                                reconcilation -> reconcilation.getSink()
                                                                .map(TransactionView.ReconciliationCodeView::of))
                                                .orElse(TransactionView.ReconciliationCodeView.NEVER),
                                transactionEntity.getReconcilation().flatMap(
                                                reconcilation -> reconcilation.getFinalStatus()
                                                                .map(TransactionView.ReconciliationCodeView::of))
                                                .orElse(TransactionView.ReconciliationCodeView.NEVER),
        ...
        public LedgerDispatchStatusView getTransactionDispatchStatus(TransactionEntity transactionEntity) {
                if (FAILED == transactionEntity.getAutomatedValidationStatus()) {
                        ...
                        if (transactionEntity.hasAnyRejection()) {
                                if (transactionEntity.getItems().stream()
                                                .anyMatch(transactionItemEntity -> transactionItemEntity.getRejection()
                                                                .stream()
                                                                .anyMatch(rejection -> rejection.getRejectionReason()
                                                                                .getSource() == ERP))) {
                                        return INVALID;
                                }
                                return PENDING;
                ...
                if (transactionEntity.hasAnyRejection()) {
                        if (transactionEntity.getItems().stream()
                                        .anyMatch(transactionItemEntity -> transactionItemEntity.getRejection().stream()
                                                        .anyMatch(rejection -> rejection.getRejectionReason()
                                                                        .getSource() == ERP))) {
                                return INVALID;
                        }
        ...
        private Set<TransactionItemView> getTransactionItemView(TransactionEntity transaction) {
                return transaction.getItems().stream().map(item -> {
                        ...
                        if (transaction.getOrganisation() != null) {
                                itemCostCenter = costCenterRepository.findById(new CostCenter.Id(
                                                ...
                                                item.getCostCenter().map(
                                                                ...
                                                                .orElse("")));
                                itemProject = projectRepository
                                                .findById(new Project.Id(transaction.getOrganisation().getId(), item
                                                                .getProject()
                                                                .map(org.cardanofoundation.lob.app.accounting_reporting_core.domain.entity.Project::getCustomerCode)
                                                                .orElse("")));
                        }
                        return new TransactionItemView(
                                        ...
                                        itemCostCenter.map(costCenter -> costCenter.getParent().map(CostCenter::getName)
                                                        .orElse(""))
                                                        .orElse(""),
                                        item.getProject().map(
                                        ...
                                        itemProject.map(project -> project.getParent().map(Project::getName).orElse(""))
                                                        .orElse(""),
                                        ...
                                        item.getDocument().map(document -> document.getCurrency().getCustomerCode())
                                                        .orElse(""),
                                        item.getDocument()
                                                        .flatMap(document -> document.getVat()
                                                                        .map(Vat::getCustomerCode))
                                                        .orElse(""),
                                        item.getDocument().flatMap(document -> document.getVat().flatMap(Vat::getRate))
                                                        .orElse(ZERO),
                                        item.getDocument()
                                                        .flatMap(d -> d.getCounterparty()
                                                                        .map(Counterparty::getCustomerCode))
                                                        .orElse(""),
                                        item.getDocument().flatMap(d -> d.getCounterparty().map(Counterparty::getType))
                                                        .isPresent() ? item
                                                                        .getDocument()
                                                                        .flatMap(d -> d.getCounterparty()
                                                                        ...
                                                                        : "",
                                        item.getDocument()
                                                        .flatMap(document -> document.getCounterparty()
                                                                        .flatMap(Counterparty::getName))
                                                        .orElse(""),
        ...
        public Map<FilterOptions, List<FilteringOptionsListResponse>> getFilterOptions(
                        ...
                        String orgId) {
                Map<FilterOptions, List<FilteringOptionsListResponse>> filterOptionsListMap = new EnumMap<>(
                                FilterOptions.class);
                for (FilterOptions filterOption : filterOptions) {
                        switch (filterOption) {

                                case USERS ->
                                        filterOptionsListMap.put(filterOption,
                                                        transactionBatchRepositoryGateway
                                                                        .findBatchUsersList(orgId).stream().map(
                                                                                        user -> FilteringOptionsListResponse
                                                                                                        .builder()
                                                                                                        .name(user)
                                                                                                        .description(user)
                                                                                                        .build())
                                ...
                                case DOCUMENT_NUMBERS ->
                                        filterOptionsListMap.put(filterOption,
                                                        transactionItemRepository.getAllDocumentNumbers().stream()
                                                                        .map(document -> FilteringOptionsListResponse
                                                                                        .builder().name(document)
                                ...
                                case COUNTER_PARTY ->
                                        filterOptionsListMap.put(filterOption,
                                                        transactionItemRepository.getAllCounterParty().stream()
                                                                        .map(document -> FilteringOptionsListResponse
                                                                                        .builder()
                                                                                        .customerCode(document.get(
                                                                                                        "customerCode"))
                                ...
                                case COUNTER_PARTY_TYPE ->
                                        filterOptionsListMap.put(filterOption, Arrays.stream(
                                                        org.cardanofoundation.lob.app.accounting_reporting_core.domain.core.Counterparty.Type
                                                                        .values())
                                                        .map(type -> FilteringOptionsListResponse.builder()
                                                                        .name(type.name())
