...
public class KeriNotificationCorrelator {
    ...
    /**
     * Eagerly purges every UNREAD notification on the agent whose own claimed route ({@code note.a.r})
     * is one of {@code routes} (stale-notification-replay fix, live Veridian evidence): {@link
     * KeriCredentialService#presentCredential} calls this immediately before sending a fresh IPEX
     * apply, on BOTH the offer and grant routes together, so debris left behind by an earlier abandoned
     * presentation attempt can never be mistaken for a reply to the apply about to be sent.
     *
     * <p><b>Root cause this closes:</b> an IPEX apply is what PROMPTS the wallet to present in the first
     * place, so any unread offer/grant already sitting on the agent's queue BEFORE that apply is even
     * sent cannot possibly be a response to it — it can only be left over from a previous, abandoned
     * attempt (a user who backed out of the wallet UI mid-flow, a prior request that timed out, ...).
     * {@link #awaitByRoute} claims the FIRST unread notification on a matching route with no other
     * correlation at all (cip113 parity — see its own javadoc), so without this purge a live run was
     * observed to claim exactly such a leftover, admit it, and have the wallet reject the resulting
     * duplicate/invalid admit outright — meanwhile the real apply's own genuine reply is never processed
     * at all. Purging immediately before the apply is sent guarantees the very next {@link
     * #awaitByRoute} call can only ever claim the reply THIS apply actually elicits: the wallet only
     * ever grants/offers in response to an apply we send, and the apply this purge guards is sent
     * strictly AFTER the purge completes — so no race is possible where a fresh, legitimate reply lands
     * in the purge's own window and is discarded.
     *
     * <p>Read notifications are deliberately left untouched: {@link #isUnreadRouteMatch} already
     * excludes them, and neither {@link #awaitByRoute} nor {@link #awaitCorrelated} can ever claim an
     * already-read notification in the first place (same {@code isUnreadRouteMatch} gate) — purging one
     * would accomplish nothing.
     *
     * <p>Reuses this class's own list/parse machinery (the same {@code notifications().list()} call and
     * parse {@link #pollOnceByRoute} itself uses) plus {@link #isUnreadRouteMatch} and {@link
     * #markAndDelete} — this is the eager counterpart of the claim {@code awaitByRoute} performs lazily
     * on arrival. A transport or parse failure listing notifications is logged and treated as "nothing
     * to purge" (returns 0) — the same tolerant treatment every other poll in this class gives a failed
     * {@code list()}, never worth failing the whole {@code presentCredential} call over. A {@link
     * #markAndDelete} failure on one particular notification (agent hiccup mid-purge) is caught and
     * logged the same way, rather than aborting the rest of the purge or propagating out of {@code
     * presentCredential} — every other agent interaction in that caller is already guarded and translated
     * into a controlled failure, and an unguarded purge would be the one exception to that rule.
     *
     * <p><b>Known limitation, shared with {@code awaitByRoute} (not introduced by this method):</b> like
     * every route-only claim in this class, this has no sender/thread correlation, so on the rare
     * overlap of two ceremonies concurrently waiting on the same routes against the same shared agent
     * queue, this could purge a genuine, not-yet-polled reply belonging to the OTHER ceremony rather than
     * a truly stale one — the same cross-ceremony risk the class javadoc already documents for {@code
     * awaitByRoute} itself (route-only matching was a deliberate, user-directed choice over the stricter,
     * but less wallet-compatible, {@link #awaitCorrelated}). The victim ceremony simply times out and can
     * be retried; nothing is corrupted.
     *
     * @return the number of notifications purged.
     */
    public int purgeUnclaimed(List<String> routes) {
        ...
        try {
            ...
            log.warn("Failed to list KERI notifications while purging stale unclaimed notifications, "
                    + "skipping purge: {}", e.getMessage());
            return 0;
        }
        ...
        try {
            ...
            log.error("Failed to parse the KERI notification list while purging stale unclaimed "
                    + "notifications, skipping purge: {}", e.getMessage());
            return 0;
        }
        ...
        for (Notification note : notes) {
            ...
            try {
                ...
                // Best-effort, same tolerance as the list()/parse failures above: one notification the
                // agent won't currently let us mark/delete must not abort the rest of the purge, and
                // must never propagate out of presentCredential (unlike every OTHER agent call in that
                // method, this one has no try/catch of its own at the call site).
                log.warn("Failed to purge stale KERI notification {}, leaving it for a later attempt: {}",
                        note.i, e.getMessage());
            }
        }
        log.info("purged {} stale unclaimed offer/grant notification(s) before sending apply", purged);
        return purged;
    }
