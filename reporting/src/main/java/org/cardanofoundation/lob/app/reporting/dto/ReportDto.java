...
public class ReportDto {
    ...
    @Schema(description = "Interval type", example = "QUARTERLY", allowableValues = {"MONTHLY", "QUARTERLY", "YEARLY"}, required = true)
    private String intervalType;

    @Schema(description = "Period number (1-12 for MONTHLY, 1-4 for QUARTERLY, null for YEARLY)", example = "1", nullable = true)
    private Short period;

    @Schema(description = "Year for the report", example = "2024", required = true)
    private Short year;

    @NotNull(message = "Data mode is required (GENERATED or USER)")
    @Schema(description = "Data mode indicating how the report data is populated", example = "GENERATED", allowableValues = {"GENERATED", "USER"}, required = true)
