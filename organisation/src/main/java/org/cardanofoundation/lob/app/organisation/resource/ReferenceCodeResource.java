...
public class ReferenceCodeResource {
    ...
    public ResponseEntity<?> upsertReferenceCode(@PathVariable("orgId") @Parameter(example = "75f95560c1d883ee7628993da5adf725a5d97a13929fd4f477be0faf5020ca94") String orgId,
        ...
        Optional<ReferenceCodeView> referenceCode = referenceCodeService.upsertReferenceCode(orgId, referenceCodeUpdate);
        if (referenceCode.isEmpty()) {
            ThrowableProblem issue = Problem.builder()
                    .withTitle("REFERENCE_CODE_NOT_FOUND")
                    .withDetail(STR."Unable to find refernce code by Id: \{orgId} and \{referenceCodeUpdate.getReferenceCode()}")
                    .withStatus(Status.NOT_FOUND)
                    .build();
